...
test("Parse Bitbucket issue", (t) => {
  t.deepEqual(
    issueParser("Bitbucket")("Fix #1 reSOLved #2 CLOSES #3 fix o/r#4 #5 o/r#6 fixing #7 /duplicate #8 @user"),
    {
      actions: {
        close: [
          { raw: "Fix #1", action: "Fix", slug: undefined, prefix: "#", issue: "1" },
...
test("Exclude HTML comments", (t) => {
  ...
  t.deepEqual(issueParser("github")("Fix #1 <!-- Fixes #2 --> #3").refs, [
    { issue: "3", slug: undefined, prefix: "#", raw: "#3" },
  ]);
...
--> Fix #4`).actions.close,
  ...
  t.deepEqual(
    issueParser("github")("Fix #1 <!-- Fix #2 --> Fix #3 <!-- Fix #4 --> #5 <!-- Fix #6 --> #7").actions.close,
    [
      { issue: "1", action: "Fix", slug: undefined, prefix: "#", raw: "Fix #1" },
      { issue: "3", action: "Fix", slug: undefined, prefix: "#", raw: "Fix #3" },
    ]
...
test("Empty options", (t) => {
  ...
  t.deepEqual(
    issueParser({ referenceActions: ["", "", "fix"], issuePrefixes: ["", "#"], mentionsPrefixes: ["@", ""] })(
      "Fix #1,@user"
    ),
    {
      refs: [],
